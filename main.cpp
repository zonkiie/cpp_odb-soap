...
int start_mt_queue(int argc, char **argv) 
{
	...
	soap_main->fget = http_get;
	soap_main->bind_flags |= SO_REUSEADDR | SO_REUSEPORT;
	soap_main->authrealm = "mysoap";
	if (argc < 2) // no args: assume this is a CGI application 
	{
		soap_serve(soap_main);   // serve request, one thread, CGI style 
		soap_destroy(soap_main); // delete allocated class instances 
		soap_end(soap_main);     // delete allocated data and temporaries 
	} 
	else 
	{
		...
		while(1)
		{
			...
			switch(c)
			{
				case 0:
				{
					...
					if(!strcmp(oname, "authrealm")) soap_main->authrealm = strdupa(optarg);
			...
			if(f != NULL)
			{
				...
				while((linelen = ::getline(&line, &n, f)) >= 0)
				{
					if(parse_configstring(line, key, value) >= 0)
					{
						...
						if(!strcmp(key, "authrealm") && serverid == NULL) soap_main->authrealm = strdupa(value);
		...
		if(ssl)
		{
			...
			if (soap_ssl_server_context(soap_main, 
				SOAP_SSL_DEFAULT, 
			...
			{
				soap_print_fault(soap_main, stderr); 
				exit(EXIT_FAILURE); 
			}
		}
		SOAP_SOCKET m, s; 
		m = soap_bind(soap_main, NULL, port, BACKLOG); 
		...
		while (run)
		{
			s = soap_accept(soap_main);
			if (soap_valid_socket(s)) 
			{
				if(debug) fprintf(stderr, "Thread %d accepts socket %d connection from IP %u.%u.%u.%u\n", i, s, (soap_main->ip>>24)&0xFF, (soap_main->ip>>16)&0xFF, (soap_main->ip>>8)&0xFF, soap_main->ip&0xFF); 
				enqueue(s);
			}
			else if (soap_main->errnum) // accept failed, try again after 1 second
			{
				soap_print_fault(soap_main, stderr); 
				sleep(1);
			} 
